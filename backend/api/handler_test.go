...
func TestListHandlers(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Store: func() store.Store {
				store := new(mockstore.MockStore)
				store.On("ListResources", mock.Anything, (&corev2.Handler{}).StorePrefix(), mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
					arg := args.Get(2).(*[]*corev2.Handler)
					*arg = []*corev2.Handler{defaultHandler}
				}).Return(nil)
				return store
			},
			Auth: func() authorization.Authorizer {
...
func TestCreateHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Store: func() store.Store {
				store := new(mockstore.MockStore)
				store.On("CreateResource", mock.Anything, defaultHandler).Return(nil)
				return store
			},
			Auth: func() authorization.Authorizer {
				auth := &mockAuth{
...
func TestUpdateHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Store: func() store.Store {
				store := new(mockstore.MockStore)
				store.On("CreateOrUpdateResource", mock.Anything, defaultHandler).Return(nil)
				return store
			},
			Auth: func() authorization.Authorizer {
				auth := &mockAuth{
...
func TestDeleteHandler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Store: func() store.Store {
				store := new(mockstore.MockStore)
				store.On("DeleteResource", mock.Anything, "handlers", "default").Return(nil)
				return store
			},
			Auth: func() authorization.Authorizer {
				auth := &mockAuth{
