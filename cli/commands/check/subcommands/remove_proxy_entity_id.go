...
// RemoveProxyEntityNameCommand adds a command that allows a user to remove the
// proxy entity name of a check
func RemoveProxyEntityNameCommand(cli *cli.SensuCli) *cobra.Command {
	cmd := &cobra.Command{
		Use:          "remove-proxy-entity-name [NAME]",
		Short:        "removes proxy entity name from a check",
		SilenceUsage: false,
