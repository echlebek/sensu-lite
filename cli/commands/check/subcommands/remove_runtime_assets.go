...
// RemoveRuntimeAssetsCommand adds a command that allows a user to remove the
// runtime assets of a check
func RemoveRuntimeAssetsCommand(cli *cli.SensuCli) *cobra.Command {
	cmd := &cobra.Command{
		Use:          "remove-runtime-assets [NAME]",
		Short:        "removes runtime assets from a check",
		SilenceUsage: false,
