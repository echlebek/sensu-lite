...
func DeregisterCommand(cli *cli.SensuCli) *cobra.Command {
	return &cobra.Command{
		...
		RunE:  runDeregister(cli.Client, cli.Config.Namespace()),
	}
}

func runDeregister(client client.APIClient, namespace string) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		name := args[0]
		if err := client.DeregisterExtension(name, namespace); err != nil {
			return err
		}
